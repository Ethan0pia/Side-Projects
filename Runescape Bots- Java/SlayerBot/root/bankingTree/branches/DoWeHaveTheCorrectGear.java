    ...
    public boolean validate() {
        ...
        for(int i=0;i<12;i++){
            if(gear[i]!=null||i==11){
                ...
                boolean equipmentContains = Equipment.contains(gear[i]);
                if(i==11 && !bankContains && !inventoryContains && !equipmentContains){
                    Environment.getLogger().severe("You do not own the primary weapon for this preset.");
                    bot.stop();
                }
                if((bankContains || inventoryContains) && !equipmentContains){
                    SpriteItem item;
