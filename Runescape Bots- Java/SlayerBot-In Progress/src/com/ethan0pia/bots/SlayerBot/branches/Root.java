...
public class Root extends BranchTask {
    ...
    public Root(GoodAssSlayerBot Bot){
        this.Bot=Bot;
        task = new HaveTask(Bot);

        Execution.delayUntil(() -> { player = Players.getLocal(); return player != null;}, 2000);
        while(startSlayXP==0) {
    ...
    public boolean validate() {
        //checks player every loop to cut down on bots
        ...
        if(Bot.infoUI!=null) {
            ...
            Bot.infoUI.update1(tasksComplete, Bot.gpGained, gph, att, attXP, slay, slayXP, str, strXP, def, defXP
                    , mage, mageXP, range, rangeXP, hp, hpXP, pray, prayXP, Bot.mobList.getTaskName(task),
            ...
            if (LootInventory.isEnabled()) {
                System.out.println("Loot inventory is enabled. Please turn it off.");
                Bot.stop();
            }
        }
    ...
    public TreeTask successTask() {
        ...
        if (Bot.stopWatch.getRuntime() > 600000)
            Bot.stop();
